...
data "aws_iam_policy_document" "aws_load_balancer_controller_assume_role_policy" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${var.eks.create ? replace(local.eks.eks_oidc_url, "https://", "") : ""}:sub"
      values   = ["system:serviceaccount:kube-system:aws-load-balancer-controller"]
    }
    condition {
      test     = "StringEquals"
      variable = "${var.eks.create ? replace(local.eks.eks_oidc_url, "https://", "") : ""}:aud"
      values   = ["sts.amazonaws.com"]
    }

    principals {
      identifiers = [local.eks.eks_oidc_arn]
      type        = "Federated"
    }
  }
}
